...
import { CaseClient, Evidence, Clue } from './types';

export interface GameState {
  currentScene: string | null;
  foundClues: string[];
  analyzedEvidence: string[];
  clueConnections: Array<{
    from: string;
    to: string;
    reason: string;
    timestamp: Date;
  ...
  investigationPhase: 'initial_exploration' | 'evidence_gathering' | 'pattern_analysis' | 'theory_development';
}

export interface BatchAnalysisResult {
  objects_detected: string[];
  text_excerpts: string[];
  potential_links: string[];
  anomalies: string[];
  initial_theories: string[];
...
export interface SmartHint {
  hint: string;
  confidence: number;
  category: 'examination' | 'connection' | 'reasoning' | 'general';
}
...
export class WebAIDetective {
  ...
  async analyzeAllEvidence(caseData: CaseClient): Promise<BatchAnalysisResult> {
  ...
  async generateDetectiveNotes(gameState: GameState, caseData: CaseClient): Promise<DetectiveNotes> {
    const prompt = this.buildDetectiveNotesPrompt(gameState, caseData);
  ...
  async generateSmartHint(gameState: GameState, userMessage: string, caseData: CaseClient): Promise<SmartHint> {
    const prompt = this.buildSmartHintPrompt(gameState, userMessage, caseData);
  ...
  async generateReasoningMap(gameState: GameState, caseData: CaseClient): Promise<ReasoningMap> {
    const prompt = this.buildReasoningMapPrompt(gameState, caseData);
  ...
  async generateCaseRecap(gameState: GameState, caseData: CaseClient): Promise<CaseRecap> {
    const prompt = this.buildCaseRecapPrompt(gameState, caseData);
  ...
  async chatWithDetective(message: string, gameState: GameState, caseData: CaseClient): Promise<{
    response: string;
    suggestions: string[];
  }> {
    const prompt = this.buildChatPrompt(message, gameState, caseData);
  ...
  private buildBatchEvidenceAnalysisPrompt(evidence: Evidence[], caseData: CaseClient): string {
...
Respond with a JSON object containing:
{
  ...
  private buildDetectiveNotesPrompt(gameState: GameState, caseData: CaseClient): string {
    const clueList = gameState.foundClues.map(clueId => {
      const clue = caseData.clues.find(c => c.id === clueId);
      return `- ${clue?.label || clueId}`;
    }).join('\n');
    
    const connectionList = gameState.clueConnections.map(conn => 
      `- ${conn.from} → ${conn.to}: ${conn.reason}`
    ).join('\n');
...
- Current Scene: ${gameState.currentScene || 'No scene selected'}
- Phase: ${gameState.investigationPhase}
- Clues Found: ${gameState.foundClues.length}/${caseData.clues.length}
- Evidence Analyzed: ${gameState.analyzedEvidence.length}/${caseData.evidence.length}
...
Keep the tone professional but personal, like a detective writing in their case journal.`;
  ...
  private buildSmartHintPrompt(gameState: GameState, userMessage: string, caseData: CaseClient): string {
    const clueList = gameState.foundClues.map(clueId => {
      const clue = caseData.clues.find(c => c.id === clueId);
      return `- ${clue?.label || clueId}`;
    }).join('\n');
...
CURRENT STATE:
- Scene: ${gameState.currentScene || 'No scene selected'}
- Phase: ${gameState.investigationPhase}
- Clues Found: ${gameState.foundClues.length}/${caseData.clues.length}
- Evidence Analyzed: ${gameState.analyzedEvidence.length}/${caseData.evidence.length}
...
Keep it encouraging and detective-like.`;
  ...
  private buildReasoningMapPrompt(gameState: GameState, caseData: CaseClient): string {
    const clueList = gameState.foundClues.map(clueId => {
      const clue = caseData.clues.find(c => c.id === clueId);
      return `- ${clue?.label || clueId}`;
    }).join('\n');
...
Format as a structured analysis that can be displayed as a text-based reasoning tree.`;
  ...
  private buildCaseRecapPrompt(gameState: GameState, caseData: CaseClient): string {
    const clueList = gameState.foundClues.map(clueId => {
      const clue = caseData.clues.find(c => c.id === clueId);
      return `- ${clue?.label || clueId}`;
    }).join('\n');
...
- Clues Found: ${gameState.foundClues.length}/${caseData.clues.length}
- Evidence Analyzed: ${gameState.analyzedEvidence.length}/${caseData.evidence.length}
- Scenes Unlocked: ${gameState.currentScene ? '1' : '0'}
...
Make it clear and organized for the player to understand their progress.`;
  ...
  private buildChatPrompt(message: string, gameState: GameState, caseData: CaseClient): string {
    const clueList = gameState.foundClues.map(clueId => {
      const clue = caseData.clues.find(c => c.id === clueId);
      return `- ${clue?.label || clueId}`;
    }).join('\n');
    
    const recentMessages = gameState.conversationHistory.slice(-5);
    const conversationContext = recentMessages.map(entry => {
...
- Scene: ${gameState.currentScene || 'No scene selected'}
- Phase: ${gameState.investigationPhase}
- Progress: ${gameState.foundClues.length} clues, ${gameState.analyzedEvidence.length} evidence analyzed
...
Respond as Inspector Gemini. Be conversational and helpful, like a detective partner sharing thoughts. Keep responses short and natural.`;
  ...
  // Parsing methods
  private parseBatchAnalysis(text: string): BatchAnalysisResult {
    try {
  ...
  private parseDetectiveNotes(text: string): DetectiveNotes {
    return {
      notes: text,
      timestamp: new Date(),
  ...
  private parseSmartHint(text: string): SmartHint {
    return {
      hint: text,
      confidence: 0.8,
  ...
  private parseReasoningMap(text: string): ReasoningMap {
    try {
      const jsonMatch = text.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        return JSON.parse(jsonMatch[0]);
  ...
  private parseCaseRecap(text: string): CaseRecap {
    return {
      recap: text,
  ...
  private categorizeHint(text: string): 'examination' | 'connection' | 'reasoning' | 'general' {
  ...
  private extractTheories(text: string): string[] {
    const theories = [];
    const lines = text.split('\n');

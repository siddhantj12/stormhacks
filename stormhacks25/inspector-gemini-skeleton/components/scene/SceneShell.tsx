...
export default function SceneShell({ caseId, sceneId }: { caseId: string, sceneId: string }) {
  ...
  useEffect(() => {
    getCaseClient(caseId).then(data => {
      loadCase(data);
      setIsLoading(false);
    });
  }, [caseId, loadCase]);
  ...
  return (
    <main className="w-screen h-screen flex flex-col bg-black">
      ...
      <div className="flex-1 relative">
        <SceneCanvas scene={currentScene} onInteract={handleInteraction} />
      </div>
